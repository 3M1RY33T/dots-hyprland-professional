...
import { DeepSeekView, deepseekCommands, sendMessage as deepseekSendMessage, deepseekTabIcon } from './apis/deepseek.js';
...
export const chatEntry = TextView({
  ...
  setup: (self) => self
    ...
    .hook(DeepSeekService, (self) => {
      if (APIS[currentApiId].name != 'Assistant (DeepSeek)') return;
      self.placeholderText = (DeepSeekService.key.length > 0
        ? getString('Message DeepSeek...')
        : getString('Enter API Key for DeepSeek...'));
    }, 'hasKey')
...
const APILIST = {
  ...
  'deepseek': {
    ...
    "sendCommand": deepseekSendMessage,
    "contentWidget": DeepSeekView(chatEntry),
    "commandBar": deepseekCommands,
    "tabIcon": deepseekTabIcon,
    "placeholderText": getString('Message DeepSeek...'),
  },
  'waifu': {
    "name": 'Waifus',
